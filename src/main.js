...
import {receiveTrade, completeTrade, denyTrade, getPlayerCharacters} from "./lets-store-stuff-core.js"
...
Hooks.once("setup", async function () {
    ...
    console.log("Let's Store Stuff 5e Loaded");
});
...
Hooks.on("renderActorSheet5eCharacter", async function (sheet, element, character) {
    ...
    try {
        ...
        console.error("Let's Store Stuff 5e | Failed to inject currency icon onto character sheet.");
    }
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            console.error("Let's Store Stuff 5e | Failed to inject onto item: ", items[i]);
        }
    }

    console.log("Let's Store Stuff 5e | Added trade icons to sheet for actor " + actorId);
});
...
function onItemTradeClick(event) {
    ...
    if (characters.length === 0) {
        ui.notifications.warn("No player chest available to send to.")
    }
    else {
